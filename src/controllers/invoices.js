...
export default function () {
  ...
  const renderInvoice = (store, order) => {
    ...
    callApi(`customers/${buyer._id}.json`, 'GET', (err, buyer_extra) => {
      if (err) {
        console.error(err)
        app.toast()
      } else {
          ...
          <div class="row px-3 pt-4 fs-15">
            ...
            <div class="col-md-3 ml-md-auto text-md-right">
              <div class="fs-17">
                ...
                ${(buyer && buyer_extra.birth_date ? `<span>Dt. de Nasc.: ${birthDate(buyer_extra)}</span><br>` : '')}
                ${(buyer && buyer.gender ? `<span>Gênero: ${buyer.gender === 'm' ? 'Masculino' : 'Feminino'}</span><br>` : '')}
                ${(buyer
